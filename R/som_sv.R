#Require all function presents in folder Script/clustering_feature/kohonen/R
#it also require dyn.load of kohonen.so file
...
som_sv <- function(prototype,nboot=100,xdim,ydim,t=0.95){
  if(is.null(prototype) || is.null(xdim) || is.null(ydim)){
    stop("You must insert prototypes and the som dimension (xdim,ydim)!\n")
  }else{
    ...
    gc();
    hls <- hclust(as.dist(M2diss)^2,method="ward");
    hls.clust <- cutree(hls,k=(xdim * ydim));
    center <- findCenter(DB=prototype,clust_vector=hls.clust)
    
    toRet <- list(som.res=hls,clustering = hls.clust,center=center)
    return(toRet)
  }
}
